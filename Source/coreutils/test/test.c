...
findop(const char *s)
{
	if (s[0] == '-') {
		if (s[1] == '\0')
			return NULL;
		if (s[2] == '\0')
			return bsearch(s + 1, mop2, (sizeof(mop2)/sizeof(mop2[0])),
			    sizeof(*mop2), compare1);
		else if (s[3] != '\0')
			return NULL;
		else
			return bsearch(s + 1, mop3, (sizeof(mop3)/sizeof(mop3[0])),
			    sizeof(*mop3), compare2);
	} else {
		if (s[1] == '\0')
			return bsearch(s, cop, (sizeof(cop)/sizeof(cop[0])), sizeof(*cop),
			    compare1);
